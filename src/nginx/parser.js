...
// Callback: error (string), NginxConfig.Config (object)
function parseNginxConf(_config, callback) {
	...
	if (_roots.length > 0) {
		...
		root = new NginxConfig.Location('/', metadata);
	...
	for (var i = 0, len = _locations.length; i < len; i++) {
		...
		var location = new NginxConfig.Location(_location.value, metadata);
	...
	var config = new NginxConfig.Config()
...
function parseRewrite(rewrite) {
	...
	return new NginxConfig.Rewrite(regex, replacement, flag)
}
...
function parseMetadata(node) {
	return Yaml.safeLoad(node.comments.join("\n"));
}
